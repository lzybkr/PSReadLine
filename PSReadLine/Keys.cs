    ...
    internal class PSKeyInfo : IEquatable<PSKeyInfo>
    {
        ...
        static string KeyInfoAsString(ConsoleKeyInfo key)
        {
            ...
            if (char.IsControl(c))
            {
                ...
                // 
                // We shouldn't fall to this block for known unambiguous control characters (codes from 0 to 26 aka 0x1A
                // aka ^Z). For details on this check, see issue #1393.
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && c > 26)
                {
